...
path is only a path, and there is no affront, to oneself or to others, in dropping it if that is
...
For Rye, we choose the path with heart. We select our tools deliberately, for every item we
...
In other words, Rye has a "zero technical debt" policy. We do it right the first time. This
...
code will be clear, will be correct, will be beautiful. If you don't have enough personal power,
even the most elegant design will fail.
...
You choose to do it right the first time. You choose to trust your personal power. You choose to see
the code as an offering, not just as a product.
...
- **Put a limit on everything** because, in reality, this is what we expect—everything has a limit.
  ...
  **The assemblage point of your code is where perception is assembled.** When you put a limit on
  ...
  illuminating the energy fields that matter, making them perceivable. Without limits, the
  assemblage point is scattered, and you cannot see clearly.
...
- All memory must be statically allocated at startup. **No memory may be dynamically allocated (or
  ...
  **Static allocation is like freezing the Basin spec.** It's a commitment. It's a foundation. It's
  ...
  In Rye, we call this **Garden Allocation**: memory plots are cultivated at startup and tended
  throughout the program's lifetime. Plots grow, they don't shrink. See
  `docs/rye/0002-tend-supervisor-vision.md`.
...
- Restrict the length of function bodies to reduce the probability of poorly structured code. We
  enforce a **hard limit of 64 lines per function** (2^6, binary-aligned). This is enforced by the
  Rye compiler.
...
- **Always motivate, always say why. Never forget to say why.** Because if you explain the rationale
  ...
  its importance. For public functions, Rye requires a `/// Why:` doc comment. This is enforced by
  the compiler.
...
- **Use `rye_case` for function, variable, and file names.** rye_case is identical to
  ...
  **rye_case is like the path with heart.** It makes for a joyful journey. It makes you strong.
...
- _Order_ matters for readability (even if it doesn't affect semantics). On the first read, a file
  ...
  /// Initialize tracer.
  /// Why: Set up tracing infrastructure before any operations.
...
- Rye has named arguments through the `options: struct` pattern. Use it when arguments can be
...
- Construct larger structs _in-place_ by passing an _out pointer_ during initialization.
  ...
  **Prefer:**
  ```rye
  /// Initialize large struct in-place.
  /// Why: Avoid stack copy during initialization.
...
- Be on your guard for **[buffer bleeds](https://en.wikipedia.org/wiki/Heartbleed)**. This is a
  ...
  deterministic guarantees as required by Rye to be violated.
...
- **Hard limit all line lengths to at most 128 columns** (2^7, binary-aligned). This ensures
  ...
  comma, close your eyes and let `rye fmt` do the rest.
...
- **Hard limit all function bodies to at most 64 lines** (2^6, binary-aligned). This is enforced
  by the Rye compiler.
...
Graincards are 128×64 monospace teaching cards used throughout the grain network. All Rye code
...
This means:
- **Rye code lines**: max 128 characters per line (hard wrap, 2^7)
- **Rye functions**: max 64 lines (entire graincard height, 2^6)
...
When you write Rye code for graincards, you're writing for a specific display format. This
constraint breeds creativity - it forces you to think carefully about every line.

## Dependencies

Rye has **a "zero dependencies" policy**, apart from the Rye toolchain. Dependencies, in
...
specialized instruments each with a dedicated manual. Our primary tool is Rye. It may not be the
best for everything, but it's good enough for most things. We invest into our Rye tooling to ensure
...
For example, the next time you write a script, instead of `scripts/*.sh`, write `scripts/*.ry`.
...
Standardizing on Rye for tooling is important to ensure that we reduce dimensionality, as the team,
...
**Standardizing on Rye is like choosing the path with heart.** When you standardize, you are
...
**The recommendation for spiritual guardians is not to have any material things on which to focus
their power, but to focus it on the spirit, on the true flight into the unknown.**
...
This document is the comprehensive style guide for Rye and Grain OS development. It synthesizes:
...
**See also**: [GrainStyle](grain_style.md) for Zig-specific guidance (RyeStyle is the main doc).
