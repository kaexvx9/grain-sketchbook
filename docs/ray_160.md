...
tigerbeetle-0.16.11)
[^dcroma]: [DeepComputing DC-ROMA RISC-
```
...
```72/ 
d](https://frame.work/products/deep-computing-risc-v-mainboard)
[^framework-blog]: [Framework Blog: RISC-V Mainboard for Framework Laptop 
13](https://fram
```

```73/ 
e.work/blog/risc-v-mainboard-for-framework-laptop-13-is-
now-available)

20. **Pre-VPS Queue (Hold Tight for Atlanta)**
    1. Scaffold `src/kernel/` local
```

```74/ 
ly—`main.zig`, `syscall_table.zig`,
       `devx/abi.zig`—plus `kernel/link.ld` and `scripts/qemu_rv64.sh`
       so `zig build kernel-rv64` works befo
```

```75/ 
re the VPS unlocks.
    2. Journal bootloader research in `docs/boot/notes.md`, capturing the
       OpenSBI ➝ U-Boot baseline and Zig/Rust payload ambit
```
