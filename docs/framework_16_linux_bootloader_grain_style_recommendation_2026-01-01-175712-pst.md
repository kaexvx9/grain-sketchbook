...
**Recommendation**: **systemd-boot** (for NixOS VM) or **EFI Stub Booting** (minimal option)

**⚠️ Context**: This bootloader is for the **NixOS VM** running inside QEMU/KVM on the Ubuntu 24.04 LTS host on Framework 16, **NOT** for the Ubuntu host itself.
...
Framework 16 (x86_64 AMD, 64GB RAM)
  └─ Ubuntu 24.04 LTS (Host OS)
      └─ QEMU/KVM (Virtualization Layer)
          └─ NixOS VM (Guest OS) ← **BOOTLOADER GOES HERE**
              └─ systemd-boot or EFI Stub (Bootloader for NixOS VM)
```
...
### Option 1: systemd-boot ✅ **RECOMMENDED FOR NIXOS VM**

**Description**: Simple UEFI boot manager (formerly Gummiboot), EFI-only, no legacy BIOS support

**⚠️ Context**: This bootloader is for the **NixOS VM** running inside QEMU/KVM on the Ubuntu 24.04 LTS host, **NOT** for the Ubuntu host itself.
...
**Context**: Configure systemd-boot for **NixOS 25.11 minimal Linux kernel 6.18.2 VM** running on **Ubuntu 24.04 LTS host** on **Framework 16**.

**Steps**:
1. **Configure NixOS VM** (`configuration.nix` inside the VM):
   ```nix
   boot.loader.systemd-boot.enable = true;
   boot.loader.efi.canTouchEfiVariables = true;
...
2. **Rebuild NixOS VM** (inside the VM):
   ```bash
   sudo nixos-rebuild switch
   ```

3. **Verify Boot** (inside the VM):
   - Reboot VM and verify systemd-boot menu appears (in virtualized EFI)
   - Verify boot entries are generated correctly
   - Test booting into NixOS 25.11 (Linux kernel 6.18.2)

4. **Verify Grain Style Compliance**:
   - Check boot time (should be fast, even in VM)
...
**Context**: Configure EFI stub booting for **NixOS 25.11 minimal Linux kernel 6.18.2 VM** running on **Ubuntu 24.04 LTS host** on **Framework 16**.
...
3. **Configure Virtualized EFI Firmware** (in VM):
   - Set virtualized EFI firmware (QEMU/KVM) to boot Linux kernel 6.18.2 directly
   - Configure kernel parameters in virtualized EFI firmware

4. **Verify Boot** (inside the VM):
   - Reboot VM and verify virtualized EFI firmware boots kernel directly
   - Verify no bootloader is used (EFI stub booting)

**Timeline**: 1-2 hours (more complex virtualized EFI firmware configuration)
