...
- **Production modules** (46, 6113 lines):
  - `uart.ry` - UART driver
  - `debug.ry` - Kernel debug utilities
  - `panic.ry` - Panic handler
  - `time_source.ry` - Time source abstraction
  - `syscall_table.ry` - Syscall descriptions
  - `platform_riscv.ry` - RISC-V platform interface
  - `interrupt_types.ry` - Unified interrupt types
  - `kernel_platform.ry` - Platform abstraction
  - `process.ry` - Process context management
  - `repl.ry` - Grainscript REPL
  - `timer.ry` - Timer driver
  - `boot.ry` - Boot sequence
  - `kernel_log_buffer.ry` - Kernel log buffer
  - `exception_types.ry` - Unified exception types
  - `keyboard.ry` - Keyboard driver
  - `segment_loader.ry` - ELF segment loader
  - `mouse.ry` - Mouse driver
  - `signal.ry` - Signal handling
  - `resource_cleanup.ry` - Process resource cleanup
  - `trap.ry` - Trap handler loop
  - `platform.ry` - Platform interface
  - `basin_kernel_syscalls_stats.ry` - Stats syscalls
  - `page_fault_stats.ry` - Page fault tracking
  - `kernel_stats_aggregator.ry` - Stats aggregation
  - `cow.ry` - Copy-on-write memory
  - `elf_parser.ry` - ELF parsing
  - `network_interface_stats.ry` - Network stats
  - `memory_stats.ry` - Memory statistics
  - `process_group_stats.ry` - Process group stats
  - `udp_socket_stats.ry` - UDP socket stats
  - `scheduler_stats.ry` - Scheduler stats
  ...
  - `tcp_socket_stats.ry` - TCP socket stats
  - `network.ry` - Network interface management
  - `syscall_performance_profiler.ry` - Syscall profiling
  - `tcp_socket.ry` - TCP socket management
  - `scheduler.ry` - Process scheduler
