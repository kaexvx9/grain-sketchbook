...
test "parser parse nested html multiple levels" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    ...
    const html = "<div><span><p>Deep</p></span></div>";
    const node = try parser.parseHtml(html);
    defer {
        allocator.free(node.tag_name);
        allocator.free(node.text_content);
        ...
        for (node.children) |child| {
            ...
            for (child.children) |grandchild| {
                allocator.free(grandchild.tag_name);
                allocator.free(grandchild.text_content);
                for (grandchild.attributes) |gcattr| {
                    allocator.free(gcattr.name);
                    allocator.free(gcattr.value);
                }
                allocator.free(grandchild.attributes);
                allocator.free(grandchild.children);
            }
            allocator.free(child.children);
        }
        allocator.free(node.children);
    ...
    // Assert: Nested structure parsed correctly
    try testing.expectEqualStrings("div", node.tag_name);
    std.debug.assert(node.children.len > 0);
    std.debug.assert(node.children[0].children.len > 0);
}

test "parser parse self closing tag" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    ...
    // Assert: Self-closing tag parsed
    try testing.expectEqualStrings("div", node.tag_name);
    std.debug.assert(node.children.len > 0);
    try testing.expectEqualStrings("br", node.children[0].tag_name);
}

test "parser parse boolean attribute" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    ...
    // Assert: Boolean attribute parsed
    try testing.expectEqualStrings("input", node.tag_name);
    std.debug.assert(node.attributes.len > 0);
    ...
    std.debug.assert(found_disabled);
}

test "parser parse unquoted attribute" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    ...
    // Assert: Unquoted attribute parsed
    try testing.expectEqualStrings("div", node.tag_name);
    std.debug.assert(node.attributes.len > 0);
...
test "parser compute style with class selector" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    ...
    // Assert: Class selector matched
    std.debug.assert(style.len > 0);
    
    var found_color = false;
    for (style) |decl| {
        if (std.mem.eql(u8, decl.property, "color")) {
            try testing.expectEqualStrings("blue", decl.value);
            found_color = true;
            break;
        }
    }
...
test "parser compute style with id selector" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    ...
    // Parse CSS with ID selector
    const css = "#main { margin: 10px; }";
    const rules = try parser.parseCss(css);
    defer {
        for (rules) |rule| {
            allocator.free(rule.selector);
    ...
    // Assert: ID selector matched
    std.debug.assert(style.len > 0);
    
    var found_margin = false;
    for (style) |decl| {
        if (std.mem.eql(u8, decl.property, "margin")) {
            try testing.expectEqualStrings("10px", decl.value);
            found_margin = true;
            break;
        }
    }
    std.debug.assert(found_margin);
}

test "parser compute style specificity id beats class" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    ...
    // Parse HTML with both class and ID
    ...
    // Parse CSS with conflicting rules (ID should win)
    const css = ".container { color: blue; } #main { color: red; }";
    const rules = try parser.parseCss(css);
    defer {
        for (rules) |rule| {
            allocator.free(rule.selector);
    ...
    // Assert: ID selector wins (higher specificity)
    std.debug.assert(style.len > 0);
...
test "parser parse css tag class selector" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    ...
    const css = "div.container { padding: 20px; }";
    const rules = try parser.parseCss(css);
    defer {
        for (rules) |rule| {
            allocator.free(rule.selector);
    ...
    // Assert: Tag+class selector parsed
    std.debug.assert(rules.len > 0);
    try testing.expectEqualStrings("div.container", rules[0].selector);
}

test "parser compute style with tag class selector" {
    var arena = std.heap.ArenaAllocator.init(testing.allocator);
    defer arena.deinit();
    const allocator = arena.allocator();
    ...
    // Parse HTML with matching tag and class
    const html = "<div class=\"container\">Content</div>";
    const node = try parser.parseHtml(html);
    defer {
        allocator.free(node.tag_name);
    ...
    // Parse CSS with tag+class selector
    const css = "div.container { padding: 20px; }";
    const rules = try parser.parseCss(css);
    defer {
        for (rules) |rule| {
    ...
    // Assert: Tag+class selector matched
    std.debug.assert(style.len > 0);
    
    var found_padding = false;
    for (style) |decl| {
        if (std.mem.eql(u8, decl.property, "padding")) {
            try testing.expectEqualStrings("20px", decl.value);
            found_padding = true;
            break;
        }
    }
