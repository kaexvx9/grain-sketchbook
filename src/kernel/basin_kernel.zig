...
pub fn handle_syscall(
    ...
    fn syscall_channel_send(
        ...
        const MAX_MSG_SIZE: u32 = 4096; // Matches channel.zig MAX_MESSAGE_SIZE
        if (data_len == 0) {
    ...
    fn syscall_sysinfo(
        ...
        const MAX_PAGES_LOCAL: u32 = 1024; // From memory.zig
        const PAGE_SIZE_LOCAL: u64 = 4096;
        const allocated_pages = self.memory_pool.allocated_pages;
        // Use saturating subtract to prevent underflow if allocated_pages > MAX_PAGES_LOCAL
        const available_pages: u32 = if (allocated_pages > MAX_PAGES_LOCAL) 0 else MAX_PAGES_LOCAL - allocated_pages;
        const available_memory: u64 = @as(u64, available_pages) * PAGE_SIZE_LOCAL;
