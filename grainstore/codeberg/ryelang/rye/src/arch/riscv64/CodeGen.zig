...
fn gen(func: *Func) !void {
    ...
    if (fn_info.cc != .naked) {
        ...
        // Why: Handle stack frame adjustment. For values > 2047, use multi-instruction sequence.
        const stack_adjust = frame_layout.stack_adjust;
        const max_i12: i32 = 2047;
        if (stack_adjust <= max_i12 and stack_adjust >= -2048) {
            // Small frame: single addi instruction
            ...
            // Large frame: lui + addi + sub sequence
            // Load -stack_adjust into temporary register, then sub from sp
            const temp_reg, const temp_lock = try func.allocReg(.int);
            defer func.register_manager.unlockReg(temp_lock);
            
            const neg_adjust = -stack_adjust;
            const lo12: i12 = @truncate(neg_adjust);
            const carry: i32 = if (lo12 < 0) 1 else 0;
            const hi20: i20 = @truncate((neg_adjust >> 12) +% carry);
            ...
            // sub sp, sp, temp_reg (which contains -stack_adjust, so this adds stack_adjust)
            func.mir_instructions.set(backpatch_stack_alloc, .{
                .tag = .sub,
                .data = .{ .r_type = .{
                    .rd = .sp,
        ...
        // Why: stack_adjust is already validated to fit in 12 bits by computeFrameLayout().
        func.mir_instructions.set(backpatch_fp_add, .{
            .tag = .addi,
            .data = .{ .i_type = .{
                .rd = .s0,
                .rs1 = .sp,
                .imm12 = Immediate.s(@intCast(stack_adjust)),
            } },
        });
        // Why: stack_adjust is already validated to fit in 12 bits by computeFrameLayout().
...
fn computeFrameLayout(func: *Func) !FrameLayout {
    ...
    // Why: Stack frame size can exceed 12-bit immediate. We handle both cases:
    // - Small frames (<= 2047): Use single addi instruction
    // - Large frames (> 2047): Use lui + addi + sub sequence (handled in codegen)
    // No need to fail here - codegen will handle large values correctly.

    return .{
        .stack_adjust = acc_frame_size,
